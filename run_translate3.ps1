    ...
    if ($langs.ContainsKey($lang)) {
        ...
        if (Test-Path $path) {
            ...
            for ($i = 0; $i -lt $fr_keys.Length; $i++) {
                ...
                if ($content.Contains($fr_val) -and ($fr_val -ne "")) {
                    $content = $content.Replace($fr_val, $target_val)
                }
            }

            # Extra fixes for the manually tweaked French strings not in the exact dictionary keys
            
            # Régime pour Diabétiques
            $content = $content.Replace("'Régime pour Diabétiques'", "'" + $translations[47].Replace("Dieta Completa para Combater o Diabetes", $translations[47]).Replace("'Dieta Completa para Combater o Diabetes'", $translations[47]).Replace($fr_keys[47], $translations[47]) + "'")
            # Wait, index 47 is 'Dieta Completa para Combater o Diabetes' equivalent in target language.

            # We can map them directly:
            if ($lang -eq "italiano") {
                $content = $content -replace "Régime pour Diabétiques", "Dieta Completa per Combattere il Diabete"
                $content = $content -replace "Livre audio du 'Dieta Completa per Combattere il Diabete Après 60 ans'", "Audiolibro della 'Dieta Completa per Combattere il Diabete dopo i 60 anni'"
                $content = $content -replace "Livre audio du 'Régime pour Diabétiques Après 60 ans'", "Audiolibro della 'Dieta Completa per Combattere il Diabete dopo i 60 anni'"
                $content = $content -replace "de reposer mes yeux et d'écouter simplement de vrais enseignements nutritionnels. Gérer le diabète est maintenant", "di riposare gli occhi e ascoltare semplicemente i veri insegnamenti di nutrizione. Gestire il diabete ora è diventato"
                $content = $content -replace "`"J'hésitais, mais les conseils quotidiens dans le groupe et les audios sont formidables. Écouter les", "`"Ero in dubbio, ma i consigli quotidiani nel gruppo e gli audio sono meravigliosi. Ascoltare le"
                $content = $content -replace "recettes à voix haute au lieu de me concentrer sur un manuel a rapidement changé ma routine en cuisine de manière", "ricette ad alta voce invece di concentrarsi su un manuale ha presto cambiato in positivo la mia routine in cucina in modo"
                $content = $content -replace "Page de commande sécurisée", "Pagina di ordine sicura"
            }
            if ($lang -eq "alemao") {
                $content = $content -replace "Régime pour Diabétiques", "Komplette Diät zur Bekämpfung von Diabetes"
                $content = $content -replace "Livre audio du 'Komplette Diät zur Bekämpfung von Diabetes Après 60 ans'", "Hörbuch der 'Kompletten Diät zur Bekämpfung von Diabetes nach den 60ern'"
                $content = $content -replace "Livre audio du 'Régime pour Diabétiques Après 60 ans'", "Hörbuch der 'Kompletten Diät zur Bekämpfung von Diabetes nach den 60ern'"
                $content = $content -replace "de reposer mes yeux et d'écouter simplement de vrais enseignements nutritionnels. Gérer le diabète est maintenant", "meine Augen auszuruhen und mir einfach die wahren Lehren der Ernährung anzuhören. Sich um den Diabetes zu kümmern, ist jetzt"
                $content = $content -replace "`"J'hésitais, mais les conseils quotidiens dans le groupe et les audios sont formidables. Écouter les", "`"Ich war skeptisch, aber die täglichen Tipps in der Gruppe und die Audios sind wunderbar. Sich die"
                $content = $content -replace "recettes à voix haute au lieu de me concentrer sur un manuel a rapidement changé ma routine en cuisine de manière", "Rezepte laut anzuhören, anstatt sich auf ein Handbuch zu konzentrieren, hat meine Routine in der Küche schnell zum Positiven verändert."
                $content = $content -replace "Page de commande sécurisée", "Sichere Bestellseite"
            }
            if ($lang -eq "espanhol") {
                $content = $content -replace "Régime pour Diabétiques", "Dieta Completa para Combatir la Diabetes"
                $content = $content -replace "Livre audio du 'Dieta Completa para Combatir la Diabetes Après 60 ans'", "Audiolibro de la 'Dieta Completa para Combatir la Diabetes después de los 60'"
                $content = $content -replace "Livre audio du 'Régime pour Diabétiques Après 60 ans'", "Audiolibro de la 'Dieta Completa para Combatir la Diabetes después de los 60'"
                $content = $content -replace "de reposer mes yeux et d'écouter simplement de vrais enseignements nutritionnels. Gérer le diabète est maintenant", "descansar los ojos y solo escuchar las enseñanzas reales de nutrición. Cuidar de la diabetes ahora es"
                $content = $content -replace "`"J'hésitais, mais les conseils quotidiens dans le groupe et les audios sont formidables. Écouter les", "`"Estaba en duda, pero los consejos diarios en el grupo y los audios son maravillosos. Escuchar las"
                $content = $content -replace "recettes à voix haute au lieu de me concentrer sur un manuel a rapidement changé ma routine en cuisine de manière", "recetas en voz alta en lugar de estar enfocado en un manual cambió rápidamente mi rutina en la cocina de forma"
                $content = $content -replace "Page de commande sécurisée", "Página de pedido segura"
            }
            if ($lang -eq "holandes") {
                $content = $content -replace "Régime pour Diabétiques", "Compleet Dieet om Diabetes te Bestrijden"
                $content = $content -replace "Livre audio du 'Compleet Dieet om Diabetes te Bestrijden Après 60 ans'", "Audioboek van het 'Compleet Dieet om Diabetes te Bestrijden na je 60ste'"
                $content = $content -replace "Livre audio du 'Régime pour Diabétiques Après 60 ans'", "Audioboek van het 'Compleet Dieet om Diabetes te Bestrijden na je 60ste'"
                $content = $content -replace "de reposer mes yeux et d'écouter simplement de vrais enseignements nutritionnels. Gérer le diabète est maintenant", "mijn ogen te laten rusten en gewoon te luisteren naar de echte voedingslessen. Zorgen voor diabetes is nu"
                $content = $content -replace "`"J'hésitais, mais les conseils quotidiens dans le groupe et les audios sont formidables. Écouter les", "`"Ik twijfelde, maar de dagelijkse tips in de groep en de audio's zijn geweldig. Hardop naar de"
                $content = $content -replace "recettes à voix haute au lieu de me concentrer sur un manuel a rapidement changé ma routine en cuisine de manière", "recepten luisteren in plaats van gefocust te zijn op een handleiding veranderde al snel mijn routine in de keuken op een"
                $content = $content -replace "Page de commande sécurisée", "Veilige bestelpagina"
            }
            if ($lang -eq "polones") {
                $content = $content -replace "Régime pour Diabétiques", "Kompletna Dieta na Walkę z Cukrzycą"
                $content = $content -replace "Livre audio du 'Kompletna Dieta na Walkę z Cukrzycą Après 60 ans'", "Audiobook 'Kompletna Dieta na Walkę z Cukrzycą po 60. roku życia'"
                $content = $content -replace "Livre audio du 'Régime pour Diabétiques Après 60 ans'", "Audiobook 'Kompletna Dieta na Walkę z Cukrzycą po 60. roku życia'"
                $content = $content -replace "de reposer mes yeux et d'écouter simplement de vrais enseignements nutritionnels. Gérer le diabète est maintenant", "odpocząć oczom i słuchać tylko prawdziwych nauk żywieniowych. Dbanie o cukrzycę stało się teraz"
                $content = $content -replace "`"J'hésitais, mais les conseils quotidiens dans le groupe et les audios sont formidables. Écouter les", "`"Miałem wątpliwości, ale codzienne wskazówki w grupie i nagrania audio są wspaniałe. Słuchanie"
                $content = $content -replace "recettes à voix haute au lieu de me concentrer sur un manuel a rapidement changé ma routine en cuisine de manière", "przepisów na głos zamiast skupiania się na podręczniku szybko i pozytywnie zmieniło moją rutynę w kuchni w sposób"
                $content = $content -replace "Page de commande sécurisée", "Bezpieczna strona zamówienia"
            }
            if ($lang -eq "arabe") {
                $content = $content -replace "Régime pour Diabétiques", "نظام غذائي كامل لمكافحة مرض السكري"
                $content = $content -replace "Livre audio du 'نظام غذائي كامل لمكافحة مرض السكري Après 60 ans'", "كتاب صوتي لـ 'نظام غذائي كامل لمكافحة مرض السكري بعد الستين'"
                $content = $content -replace "Livre audio du 'Régime pour Diabétiques Après 60 ans'", "كتاب صوتي لـ 'نظام غذائي كامل لمكافحة مرض السكري بعد الستين'"
                $content = $content -replace "de reposer mes yeux et d'écouter simplement de vrais enseignements nutritionnels. Gérer le diabète est maintenant", "إراحة عيني والاستماع فقط إلى الدروس الغذائية الحقيقية. أصبحت العناية بمرض السكري الآن"
                $content = $content -replace "`"J'hésitais, mais les conseils quotidiens dans le groupe et les audios sont formidables. Écouter les", "`"كنت متردداً، لكن النصائح اليومية في المجموعة والتسجيلات الصوتية رائعة. الاستماع إلى"
                $content = $content -replace "recettes à voix haute au lieu de me concentrer sur un manuel a rapidement changé ma routine en cuisine de manière", "الوصفات بصوت عالٍ بدلاً من التركيز على دليل غير روتيني في المطبخ بشكل إيجابي وسريع. بطريقة"
                $content = $content -replace "Page de commande sécurisée", "صفحة طلب آمنة"
            }
